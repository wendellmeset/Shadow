    ...
    @Override public void tick() {
        ...
        for(double x = -10;x<11;x++) {
            for(double z = -10;z<11;z++) {
                ...
                airs = airs.stream().filter(blockPosDoubleEntry -> CoffeeClientMain.client.world.getBlockState(blockPosDoubleEntry.getKey().down()).getMaterial().blocksMovement()).collect(Collectors.toList());
                Map.Entry<BlockPos, Double> best1 = airs.stream().min(Comparator.comparingDouble(Map.Entry::getValue)).orElse(null);
                if (best1 == null) continue; // just void here, cancel
                BlockPos best = best1.getKey();
                if (CoffeeClientMain.client.world.getBlockState(best.down()).getBlock() == Blocks.TNT) continue; // already placed tnt, cancel
                Vec3d lmao = Vec3d.of(best);
                if (lmao.add(.5,.5,.5).distanceTo(CoffeeClientMain.client.player.getCameraPosVec(1)) >= 5) continue;
                if (shouldPlace(best)) {
