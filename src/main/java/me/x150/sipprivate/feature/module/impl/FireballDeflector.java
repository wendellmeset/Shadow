...
public class FireballDeflector extends Module {
    ...
    void hit(FireballEntity fe) {
        ...
        if (owner != null) {
            ...
            if (owner.equals(SipoverPrivate.client.player)) {
                return;
            }
            ...
            for (Entity entity : SipoverPrivate.client.world.getEntities()) {
                if (entity instanceof FireballEntity fe) {
                    ...
                    if (inHitRange(SipoverPrivate.client.player, fe)) {
                        Renderer.R3D.line(Utils.getInterpolatedEntityPosition(SipoverPrivate.client.player)
                                .add(0, SipoverPrivate.client.player.getHeight() / 2, 0), Utils.getInterpolatedEntityPosition(fe).add(0, fe.getHeight() / 2, 0), Color.RED, matrices);
                    }
                }
            }
        }
