...
public class AutoLavacast extends Module {
    ...
    EnumSetting<Mode> mode = this.config.create(new EnumSetting.Builder<>(Mode.Bypass)
            .name("Mode")
    ...
    BlockPos getNextPosition() {
        ...
        while ((y + start.getY()) < CoffeeClientMain.client.world.getTopY()) {
            ...
            if (CoffeeClientMain.client.world.getBlockState(next).getMaterial().isReplaceable()) return next;
            y++;
        }
        return null;
    }
    ...
    public void onFastTick() {
        if (mode.getValue() == Mode.Fast && !timer.hasExpired(100)) return;
        timer.reset();
        ...
        if (mode.getValue() == Mode.Bypass) {
            ...
            if (((CoffeeClientMain.client.player.horizontalCollision && moveForwards) || CoffeeClientMain.client.player.getBoundingBox().intersects(Vec3d.of(next), Vec3d.of(next).add(1, 1, 1))) && CoffeeClientMain.client.player.isOnGround()) {
    ...
    @Override
    public void enable() {
        if (original == null) original = CoffeeClientMain.client.player.input;
        if (mode.getValue() == Mode.Bypass) CoffeeClientMain.client.player.input = new ListenInput();
    ...
    public void onWorldRender(MatrixStack matrices) {
        ...
        if (next != null) Renderer.R3D.renderOutline(Vec3d.of(next), new Vec3d(1, 1, 1), Color.BLUE, matrices);
    }
