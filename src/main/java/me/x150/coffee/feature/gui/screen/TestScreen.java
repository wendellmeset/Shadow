...
//            FontRenderers.getRenderer().drawString(matrices, "Shit", contentPos.x, contentPos.y, 0xFFFFFF, false);
//        });
        ClientFontRenderer cfr = (ClientFontRenderer) FontRenderers.getRenderer();
        char[] e = cfr.getRenderer().getRegisteredCharacters();
        int index = (int) ((System.currentTimeMillis()%(e.length*100))/100);
        char current = e[index];
        float height = cfr.getFontHeight(current+"");
        cfr.drawString(matrices,current+"",5,5,0xFFFFFF);
        Renderer.R2D.renderQuad(matrices,Color.RED,3,5,4,5+height);
        cfr.drawString(matrices,index+" of "+e.length,100,5,0xFFFFFF);
